...
namespace
{
    ...
    vector<Value *> meaningfulGepIndices(GetElementPtrInst *gep)
    {
        vector<Value *> indices = gep->getIndices();
        int trim = gep->num_addedzero;
        while (trim > 0 && !indices.empty())
        {
            indices.pop_back();
            --trim;
        }
    ...
    bool matchIndexDelta(Value *idx, Value *centerIdx, int64_t &delta)
    {
        idx = stripCopy(idx);
        centerIdx = stripCopy(centerIdx);
        if (idx == centerIdx)
        {
            delta = 0;
            return true;
        ...
        {
            if (stripCopy(bin->getLHS()) == centerIdx)
            {
                if (auto *c = dynamic_cast<ConstantInt *>(stripCopy(bin->getRHS())))
                {
                    delta = c->Value;
                    return true;
                }
            }
            if (stripCopy(bin->getRHS()) == centerIdx)
            {
                if (auto *c = dynamic_cast<ConstantInt *>(stripCopy(bin->getLHS())))
                {
                    delta = c->Value;
                    return true;
                }
    ...
    // 与 InstructionSelector::visitElementPtrInst 一致：从最内层向外累乘 dimStrides
    vector<int64_t> byteStridesForGepPointer(Value *ptrOp, size_t numIndices)
    {
        vector<int64_t> strides;
        if (numIndices == 0 || !ptrOp)
            return strides;

        vector<size_t> dimStrides;
        if (auto *ptrTy = dynamic_cast<PointerType *>(ptrOp->getType()))
        {
            if (auto *arr = dynamic_cast<ArrayType *>(ptrTy->ElementType))
                dimStrides = arr->getArrayIndices();
        }

        int elemSize = 4;
        if (!dimStrides.empty() && ptrOp->getType()->isPointerTy())
        {
            if (auto *arr = dynamic_cast<ArrayType *>(
                    dynamic_cast<PointerType *>(ptrOp->getType())->ElementType))
                elemSize = getElemSizeBytes(arr->getGroundElementType());
        }

        strides.resize(numIndices);
        int64_t elemMult = 1;
        for (int d = static_cast<int>(numIndices) - 1; d >= 0; --d)
        {
            strides[static_cast<size_t>(d)] = elemMult * elemSize;
            if (d > 0)
            {
                size_t strideIdx = static_cast<size_t>(d - 1);
                if (strideIdx < dimStrides.size())
                    elemMult *= static_cast<int64_t>(dimStrides[strideIdx]);
            }
        }
        return strides;
    }

    bool isMultiIndexRootGep(GetElementPtrInst *gep)
    {
        if (!gep || gep->getOpcode() != Opcode::GetElementPtr)
            return false;
        if (dynamic_cast<GetElementPtrInst *>(stripCopy(gep->getPointerOperand())))
            return false;
        return meaningfulGepIndices(gep).size() >= 2;
    }

    GetElementPtrInst *pickAnchorGep(BasicBlock *bb, const vector<GetElementPtrInst *> &geps)
    {
        for (auto &instPtr : bb->getInstructions())
        {
            if (auto *store = dynamic_cast<StoreInst *>(instPtr.get()))
            {
                auto *addr = stripCopy(store->getPointer());
        ...
        return geps.empty() ? nullptr : geps[0];
    }

    void hoistInstBefore(BasicBlock *bb, Instruction *inst, Instruction *before)
    {
        if (!inst || !before || inst == before)
            return;
        auto &insts = bb->getInstructions();
        Instruction *moved = nullptr;
        for (auto it = insts.begin(); it != insts.end(); ++it)
        {
            if (it->get() == inst)
            {
        ...
        for (auto it = insts.begin(); it != insts.end(); ++it)
        {
            if (it->get() == before)
            {
                insts.insert(it, std::unique_ptr<Instruction>(moved));
                return;
            }
        }
        insts.insert(insts.begin(), std::unique_ptr<Instruction>(moved));
    }

    bool processRelativeGepGroup(BasicBlock *bb, const vector<GetElementPtrInst *> &geps,
                                 ...
                                 vector<Value *> &needToDelete, bool &changed)
    {
        ...
        GetElementPtrInst *anchor = pickAnchorGep(bb, geps);
        if (!anchor)
            return false;

        vector<Value *> anchorIdx = meaningfulGepIndices(anchor);
        vector<int64_t> strides =
            byteStridesForGepPointer(anchor->getPointerOperand(), anchorIdx.size());
        if (strides.size() != anchorIdx.size())
            return false;

        Instruction *earliest = geps[0];
        for (auto &instPtr : bb->getInstructions())
        {
    ...
    found_earliest:
        hoistInstBefore(bb, anchor, earliest);
        ...
        for (auto *gep : geps)
        {
            ...
            vector<Value *> otherIdx = meaningfulGepIndices(gep);
            if (otherIdx.size() != anchorIdx.size())
                continue;

            int64_t byteOff = 0;
            bool ok = true;
            for (size_t d = 0; d < anchorIdx.size(); ++d)
            {
                int64_t delta = 0;
                if (!matchIndexDelta(otherIdx[d], anchorIdx[d], delta))
                {
                    ok = false;
                    break;
                }
                byteOff += delta * strides[d];
            }
            if (!ok || byteOff == 0)
                continue;
            ...
            auto *byteOffVal = new ConstantLong(LongType::getInstance(), byteOff);
            auto *newAddr =
                new BinaryOperator(Opcode::Addd, anchor, byteOffVal, gep->getName() + "_reloff");
            ...
            for (auto it = insts.begin(); it != insts.end(); ++it)
            {
                ...
                if (verbose)
                {
                    debugInfo << "RelativeGepOffset: " << gep->getName() << " -> addd("
                              << anchor->getName() << ", " << byteOff << "B) in " << bb->getName()
                              << "\n";
                }
                break;
            }
...
bool RelativeGepOffsetPass::runOnFunction(Function *func)
{
    bool changed = false;
    for (auto &bbPtr : func->getBasicBlocks())
    {
        BasicBlock *bb = bbPtr.get();
        std::unordered_map<Value *, vector<GetElementPtrInst *>> groups;
        for (auto &instPtr : bb->getInstructions())
        {
            auto *gep = dynamic_cast<GetElementPtrInst *>(instPtr.get());
            if (!isMultiIndexRootGep(gep))
                continue;
            Value *root = getOriginalPointerFromAddress(gep->getPointerOperand());
            groups[root].push_back(gep);
        }

        for (auto &[_, geps] : groups)
            processRelativeGepGroup(bb, geps, verbose, debugInfo, needToDelete, changed);
    }
    return changed;
}
