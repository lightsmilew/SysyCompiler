...
void RISCVBuilder::generateInstructions()
{
    ...
    // 首先处理所有全局数组（只处理一次）
    for (auto &globalVar : irModule->GlobalVariables)
    {
        if (globalVar->isArray())
        {
            selector.processGlobalArray(globalVar.get());
        }
    }
...
void InstructionSelector::visitCallInst(CallInst *inst)
{
    ...
    for (auto arg : inst->getArguments())
    {
        ...
        if (arg->getType()->isPointerTy())
        {
            ...
            auto arrayIt = arrayInfoMap.find(argName);

            if (arrayIt != arrayInfoMap.end())
            {
                // 这是一个数组参数，传递数组的基地址
                argReg = getArrayBaseAddress(arg);
            }
        }
...
void InstructionSelector::visitAllocaInst(AllocaInst *inst)
{
    ...
    // 检查分配的是否为数组类型
    if (dynamic_cast<ArrayType *>(allocatedType))
    {
        // 数组类型，使用新的数组管理
        processLocalArray(inst);
    }
...
void InstructionSelector::visitElementPtrInst(GetElementPtrInst *inst)
{
    ...
    // 1. 获取数组基地址（统一接口，自动处理全局/局部/参数数组）
    auto baseAddr = getArrayBaseAddress(inst->getPointerOperand());

    auto offsetReg = calculateArrayOffset(inst);

    // 3. 生成最终地址：baseAddr + offset
    auto resultAddr = allocateTempRegister(RegisterType::GENERAL, "gep_result");
    auto addInst = RISCVInstruction::createRType(RISCVOpcode::ADD, resultAddr, baseAddr, offsetReg);
...
shared_ptr<RISCVRegister> InstructionSelector::getOrCreateVirtualReg(Value *value)
{
    ...
    // 对于需要从栈加载的值（如指令的结果）
    StackFrame &stackFrame = currentFunc->getStackFrame();
    if (stackFrame.hasAllocation_value(valueName))
    {
        // 检查是否已经有加载过的寄存器
        ...
        // 生成从栈加载的指令
        int offset = stackFrame.getValueOffset(valueName);
...
void InstructionSelector::storeValueToStack(Value *value, shared_ptr<RISCVRegister> reg, int size)
{
    ...
    {
        // 如果还没有分配空间，现在分配
        stackFrame.allocateValueSpace(valueName, size);
    }
...
{
    auto spReg = make_shared<RISCVRegister>(RISCVRegister::PhysicalReg::SP);
    RISCVOpcode fdopcode = (reg->getType() == RegisterType::FLOAT) ? (isDouble ? RISCVOpcode::FSD : RISCVOpcode::FSW) : (isDouble ? RISCVOpcode::SD : RISCVOpcode::SW);
    RISCVOpcode sdopcode = (reg->getType() == RegisterType::FLOAT) ? (isDouble ? RISCVOpcode::FLD : RISCVOpcode::FLW) : (isDouble ? RISCVOpcode::LD : RISCVOpcode::LW);
    ...
    if (isImmediateInRange(offset))
    {
        ...
        if (isStore)
        {
            ...
            auto inst = RISCVInstruction::createSType(fdopcode, spReg, reg, offset);
            currentBB->addInstruction(inst);
        }
        else
        {

            auto inst = RISCVInstruction::createIType(sdopcode, reg, spReg, offset);
            currentBB->addInstruction(inst);
        }
    }
    else
    {
        ...
        if (isStore)
        {
            ...
            auto memInst = RISCVInstruction::createSType(fdopcode, tempReg, reg, 0);
            currentBB->addInstruction(memInst);
        }
        else
        {

            auto memInst = RISCVInstruction::createIType(sdopcode, reg, tempReg, 0);
            currentBB->addInstruction(memInst);
        }
    }
}
...
void InstructionSelector::prescanFunction()
{
    StackFrame &stackFrame = currentFunc->getStackFrame();
    ...
    // 扫描所有基本块和指令
    for (const auto &bb : irFunction->BasicBlocks)
    {
        for (const auto &inst : bb->Instructions)
        {
            ...
            case Opcode::Alloca:
                if (auto allocaInst = dynamic_cast<AllocaInst *>(instr))
                {
                    ...
                    if (allocatedType->isArrayTy())
                    {
                        ...
                        allocSize = arrayType->getArrayLength() * 4; // 数组元素大小
                    }
                    else
                    {
                        allocSize = 4;
            ...
            if (hasResult)
            {
                ...
                if (destValue)
                {
                    stackFrame.allocateValueSpace(destValue->getName(), allocSize);
                }
            }
        }
    }

    // 计算saveArgs：保存调用者寄存器的空间
    if (hasCallInst)
    {
        // 保存调用者寄存器（a0-a7, fa0-fa7）
        int intArgCount = 0;
        int floatArgCount = 0;
        int i = 0;
        ...
        for (auto &arg : irFunction->getArguments())
        {
            if (arg->getType()->isIntegerTy())
            {
                ...
                stackFrame.allocateCallerArgSpace(i);
                intArgCount++;
            }
            ...
            else
            {
                ...
                stackFrame.allocateCallerArgSpace(i, 8);
                intArgCount++;
            }
...
// 按ABI规范生成函数序言
void InstructionSelector::generateFunctionPrologue()
{
    StackFrame &stackFrame = currentFunc->getStackFrame();
    ...
    if (totalSize > 0)
    {
        ...
        if (isImmediateInRange(-totalSize))
        {
            ...
            currentFunc->getBasicBlocks()[0]->addInstruction(inst);
        }
        else
        {
            // 需要用li + add
            ...
            currentFunc->getBasicBlocks()[0]->addInstruction(liInst);

            auto addInst = RISCVInstruction::createRType(RISCVOpcode::ADD, spReg, spReg, tempReg);
            currentFunc->getBasicBlocks()[0]->addInstruction(addInst);
        ...
        if (stackFrame.raStackSize > 0)
        {
            ...
            if (isImmediateInRange(raOffset))
            {
                ...
                currentFunc->getBasicBlocks()[0]->addInstruction(swInst);
            }
            else
            {
                // 偏移量太大，需要先计算地址
        ...
        // 3. 保存调用者的参数寄存器（a0-a7, fa0-fa7）
        saveCallerArgs();
    }
}

// 按ABI规范生成函数尾声
void InstructionSelector::generateFunctionEpilogue()
{
    StackFrame &stackFrame = currentFunc->getStackFrame();
    int totalSize = stackFrame.getAlignedSize(); // 使用ABI计算的对齐大小
...
void InstructionSelector::mapArguments()
{
    ...
    int stackSize = currentFunc->getStackFrame().getAlignedSize();

    for (const auto &arg : irFunction->getArguments())
    {
        // 根据参数类型分类
        if (arg->getType()->isIntegerTy())
        {
            ...
            {
                // 超出范围的整数参数从栈获取
                stackArguments[arg->getName()] = stackSize + (intArgIndex + floatArgIndex) * 4 + ptrArgIndex * 8;
                intArgIndex++;
            }
        }
        else if (arg->getType()->isFloatTy())
...
// 保存调用者参数到栈
void InstructionSelector::saveCallerArgs()
{
    if (!hasCallInst)
    {
        return; // 如果没有call指令，则不需要保存调用者的寄存器
    }
    ...
    int floatArgCount = 0;
    auto spReg = make_shared<RISCVRegister>(RISCVRegister::PhysicalReg::SP);
    auto stackFrame = currentFunc->getStackFrame();
    for (auto &arg : irFunction->getArguments())
    {
        int isPtr = arg->getType()->isPointerTy();
        bool isFloat = arg->getType()->isFloatTy();

        int offset = stackFrame.getCallerArgOffset(i);
        if (offset == -1)
            continue;

        auto argReg = make_shared<RISCVRegister>(
            static_cast<RISCVRegister::PhysicalReg>(static_cast<int>(RISCVRegister::PhysicalReg::A0) + intArgCount));

        if (isFloat)
        {
            argReg = make_shared<RISCVRegister>(
                static_cast<RISCVRegister::PhysicalReg>(static_cast<int>(RISCVRegister::PhysicalReg::FA0) + floatArgCount));
            floatArgCount++;
        }
        else
        {
        ...
        auto op = isPtr ? RISCVOpcode::SD : isFloat ? RISCVOpcode ::FSW
                                                    : RISCVOpcode ::SW;

        if (isImmediateInRange(offset))
        {
            auto swInst = RISCVInstruction::createSType(op, spReg, argReg, offset);
            currentFunc->getBasicBlocks()[0]->addInstruction(swInst);
        }
        else
        {
            ...
            auto swInst = RISCVInstruction::createSType(op, tempReg, argReg, 0);
            currentFunc->getBasicBlocks()[0]->addInstruction(swInst);
...
void InstructionSelector::restoreCallerSavedRegisters()
{
    if (!hasCallInst)
    {
        return; // 如果没有call指令，则不需要恢复调用者的寄存器
    }
    ...
    // 定义参数寄存器序列
    vector<RISCVRegister::PhysicalReg> intArgRegs = {
    ...
    vector<RISCVRegister::PhysicalReg> floatArgRegs = {
    ...
    // 遍历当前函数的参数，恢复保存在栈上的参数到对应的寄存器
    for (auto &arg : irFunction->getArguments())
    {
        bool shouldRestore = false;
        RISCVRegister::PhysicalReg regToRestore;
        ...
        if (arg->getType()->isFloatTy())
        {
            if (floatArgCount < 8)
            {
                regToRestore = floatArgRegs[floatArgCount];
                shouldRestore = true;
                isFloatReg = true;
                loadOp = RISCVOpcode::FLW;
                floatArgCount++;
            }
        }
        else if (arg->getType()->isIntegerTy() || arg->getType()->isPointerTy())
        {
            if (intArgCount < 8)
            {
                regToRestore = intArgRegs[intArgCount];
                shouldRestore = true;
                isFloatReg = false;
                loadOp = arg->getType()->isPointerTy() ? RISCVOpcode::LD : RISCVOpcode::LW;
                intArgCount++;
            }
        }
        ...
        {
            // 检查这个参数是否有在栈上的保存
            if (stackFrame.hasAllocation_callerArg(argIndex))
            {
                // 获取参数在栈中的偏移量
                int offset = stackFrame.getCallerArgOffset(argIndex);

                // 创建目标寄存器
                auto argReg = make_shared<RISCVRegister>(regToRestore);

                if (isImmediateInRange(offset))
                {
                    // 直接从栈加载到参数寄存器
                    auto loadInst = RISCVInstruction::createIType(loadOp, argReg, spReg, offset);
                    currentBB->addInstruction(loadInst);
                }
                else
                {
                    ...
                    auto loadInst = RISCVInstruction::createIType(loadOp, argReg, tempReg, 0);
                    currentBB->addInstruction(loadInst);
...
shared_ptr<RISCVRegister> InstructionSelector::getArrayBaseAddress(Value *arrayValue)
{
    string arrayName = arrayValue->getName();
    auto it = arrayInfoMap.find(arrayName);

    if (it == arrayInfoMap.end())
    {
        throw std::runtime_error("Array not found in arrayInfoMap: " + arrayName);
    }

    ArrayInfo &info = it->second;
    auto baseReg = allocateTempRegister(RegisterType::GENERAL, "array_base_" + arrayName);

    switch (info.location)
    {
    case ArrayInfo::Location::GLOBAL:
        generateGlobalAddressLoad(baseReg, info.globalLabel);
        break;

    case ArrayInfo::Location::STACK:
        generateStackAddressLoad(baseReg, info.stackOffset);
        break;

    case ArrayInfo::Location::PARAMETER:
        // 参数数组传递的是指针，从寄存器或栈中加载指针值
        {
            auto existingReg = getOrCreateVirtualReg(arrayValue);
            auto moveInst = RISCVInstruction::createPseudo(RISCVOpcode::MV, baseReg, existingReg);
            currentBB->addInstruction(moveInst);
        }
        break;
    }
...
void InstructionSelector::generateGlobalAddressLoad(shared_ptr<RISCVRegister> reg, const string &label)
{
    // la reg, label (伪指令，等价于 auipc + addi)
    auto laInst = RISCVInstruction::createPseudoLA(reg, label);
    currentBB->addInstruction(laInst);
}

void InstructionSelector::generateStackAddressLoad(shared_ptr<RISCVRegister> reg, int offset)
{
    auto spReg = make_shared<RISCVRegister>(RISCVRegister::PhysicalReg::SP);

    if (isImmediateInRange(offset, 12))
    {
        // addi reg, sp, offset
        auto addiInst = RISCVInstruction::createIType(RISCVOpcode::ADDI, reg, spReg, offset);
        currentBB->addInstruction(addiInst);
    }
    else
    {
        // 大偏移量，先加载到临时寄存器
        auto tempReg = allocateTempRegister(RegisterType::GENERAL, "large_offset");
        generateConstantLoad(tempReg, offset);

        // add reg, sp, tempReg
        auto addInst = RISCVInstruction::createRType(RISCVOpcode::ADD, reg, spReg, tempReg);
        currentBB->addInstruction(addInst);
    }
}

shared_ptr<RISCVRegister> InstructionSelector::calculateArrayOffset(GetElementPtrInst *gepInst)
{
    auto stridePtr = gepInst->getArrayStride(); // 获取数组步长 vector<int> stride;
    auto indices = gepInst->getIndices();       // 获取索引 vector<Value*> indices;
...
void InstructionSelector::processLocalArray(AllocaInst *alloca)
{
    string arrayName = alloca->getDest()->getName();

    ArrayInfo info;
    info.location = ArrayInfo::Location::STACK;
    info.totalSize = alloca->getAllocatedSize() * 4;
    info.elementType = alloca->getGroundElementType();
    info.dimensions = alloca->getArrayEveryDimensionLength();

    // 从已经分配的栈空间中获取偏移量（prescanFunction已经处理了栈分配）
    StackFrame &stackFrame = currentFunc->getStackFrame();
...
void InstructionSelector::processParameterArray(Argument *arg)
{
    string paramName = arg->getName();
    Type *paramType = arg->getType();

    // 参数数组通过指针传递
    if (auto ptrType = dynamic_cast<PointerType *>(paramType))
    {
        ArrayInfo info;
        info.location = ArrayInfo::Location::PARAMETER;
        info.elementType = ptrType->getGroundElementType();
        info.dimensions = ptrType->getArrayIndices();
        info.totalSize = 0; // 参数数组大小在编译时未知

        arrayInfoMap[paramName] = info;
    }
}

void InstructionSelector::processGlobalArray(GlobalVariable *gvar)
{
    ArrayInfo info;
    info.location = ArrayInfo::Location::GLOBAL;
    info.globalLabel = gvar->getName();
    info.dimensions = gvar->getDims();
    info.elementType = gvar->getGroundElementType();
    info.totalSize = gvar->getTotallength() * 4; // 假设元素类型为32位整数

    arrayInfoMap[gvar->getName()] = info;
}
