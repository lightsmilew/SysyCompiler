# IRPass 设计与实现一览

本节列举 `/midend/pass` 目录下所有 IR 优化 Pass，并简要描述每个 Pass 的实现逻辑。

---

## 1. DeadCodeEliminationPass（死代码消除）

**实现逻辑**：  
- 遍历所有基本块和指令，删除无用的指令（如结果未被使用且无副作用的指令）。
- 删除不可达的基本块（无前驱且非入口块）。
- 支持多轮迭代，直到无可删除内容。

---

## 2. CommonSubexpressionEliminationPass（公共子表达式消除）

**实现逻辑**：  
- 在基本块或支配子树内，查找重复的表达式（如 a+b 多次出现）。
- 用哈希表记录已出现的表达式，将后续相同表达式替换为前者的结果。
- 需保证表达式无副作用且操作数未被修改。

---

## 3. LoopInvariantCodeMotionPass（循环不变代码外提）

**实现逻辑**：  
- 分析循环体内的指令，判断哪些指令在每次循环迭代中结果不变。
- 将这些指令移动到循环外的前置块，减少循环体内重复计算。
- 需保证外提后数据依赖和控制流正确。

---

## 4. FunctionInliningPass（函数内联）

**实现逻辑**：  
- 遍历所有函数调用点，判断被调用函数是否适合内联（如小函数、无递归等）。
- 将被调用函数的 IR 复制到调用点，替换参数和返回值，调整控制流。
- 删除已无调用的内联函数定义。

---

## 5. ConstantFoldingPass（常量折叠）

**实现逻辑**：  
- 遍历所有指令，发现操作数均为常量的表达式，直接计算结果并替换为常量。
- 支持算术、比较、类型转换等常量表达式。

---

## 6. PhiEliminationPass（Phi 指令消除）

**实现逻辑**：  
- 将 SSA 形式下的 phi 指令转换为普通的赋值或 copy 指令。
- 在前驱块插入相应的 copy，保证数据流一致。

---

## 7. LiveVariableAnalysisPass（活跃变量分析）

**实现逻辑**：  
- 对每个基本块做数据流分析，计算每条指令前后的活跃变量集合。
- 为寄存器分配、死代码消除等优化提供数据支持。

---

## 8. GEPExpansionPass（GEP 展开）

**实现逻辑**：  
- 将多维数组的 GEP（GetElementPtr）指令展开为一维链式 GEP，便于后端处理和优化。
- 递归分解多维索引，生成等价的线性地址计算。

---

## 9. CFGSimplificationPass（控制流图简化）

**实现逻辑**：  
- 合并空基本块、删除无用跳转、简化条件分支。
- 移除只有一个前驱和一个后继的中间块，优化控制流结构。

---

## 10. ArrayEliminationPass（数组消除/SRA）

**实现逻辑**：  
- 检查数组的所有 store/load 是否可用标量表达式替换（如 a[i]=A+i，a[i]只被顺序访问）。
- 若满足条件，将数组访问替换为等价的标量表达式，删除原数组定义和相关 store/load。

---

## 11. RemoveUselessWhilePass（无用循环删除）

**实现逻辑**：  
- 检查循环体是否只包含循环变量自增/自减且无副作用。
- 若是，则删除该循环，直接跳到循环出口。

---

## 12. GEPToBitCastPass（GEP转BitCast）

**实现逻辑**：  
- 将某些特殊情况下的 GEP 指令转换为更简单的 BitCast 指令，便于后端生成高效代码。

---

## 13. AddChainReductionPass（加法链归约）

**实现逻辑**：  
- 检测连续的加法链（如 a+b+c+d），将其归约为更高效的表达式或循环展开。

---

## 14. StrengthReductionPass（强度削弱）

**实现逻辑**：  
- 将高开销操作（如乘法、除法）转化为低开销操作（如加法、移位），提升运行效率。

---

## 15. LoopAnalysisPass（循环分析）

**实现逻辑**：  
- 分析函数中的所有循环，记录循环头、体、出口、归纳变量等信息，为其他 Pass 提供支持。

---

## 16. ControlFlowAnalysis（控制流分析工具）

**实现逻辑**：  
- 提供支配树、可达性、路径分析等基础算法，为 CSE、LICM、DCE 等 Pass 提供数据支持。

---

## 设计亮点

- **高度模块化**：每个 Pass 独立实现，便于组合、调试和扩展。
- **优化管道灵活**：PassManager 支持自定义优化流水线，适应不同优化等级和场景。
- **高效数据流/控制流分析**：采用高效算法（如 Lengauer-Tarjan 支配树）提升分析效率。
- **调试与可视化友好**：所有 Pass 支持详细调试输出，便于定位问题和教学演示。
- **工程与教学兼容**：既能满足工程实际需求，也适合教学展示优化原理和效果。

---

如需详细实现和用法，请查阅各 Pass 的源码与注释。